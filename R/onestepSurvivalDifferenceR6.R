  ...
  public = list(
    ...
    onestep_diff_curve = function() {
      ...
      while ((stopping >= self$tol) & (iter_count <= self$max.iter)) {
        ...
        print(stopping)
        if (stopping_prev < stopping) {
          onestepfit$epsilon.step <- -onestepfit$epsilon.step
        }
    ...
    plot_onestep_curve = function(...) {
      step_curve <- stepfun(
        x = 1:self$MOSS_A1$T.max,
        y = c(self$Psi.hat, self$Psi.hat[length(self$Psi.hat)])
      )
      # can `add`, `col`
    ...
    plot_CI_simultaneous = function(...) {
      ...
      polygon(
        c(
          1:self$MOSS_A1$T.max, rev(1:self$MOSS_A1$T.max)
        ),
        c(
          c(self$simul_CI[, "upper_CI"]), rev(c(self$simul_CI[, "lower_CI"]))
        ),
        col = rgb(0.7, 0.7, 0.7, 0.4),
        border = NA,
        ...
